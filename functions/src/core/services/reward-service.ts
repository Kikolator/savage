...
import { PayoutChannel, ReferralStatus, ReferrerType, RewardStatus } from "../data/enums";
...
export class RewardService {
    ...
    public async createRewardsForConversion(
        ...
        await this.firestoreService.runBatch(async (batch) => {
            for (const r of rewards) {
                const docRef = this.firestoreService.createReference(this.rewardsCollection);
                batch.set(docRef, r.toDocumentData());
            }
        });
        logger.info(`Created ${rewards.length} reward(s) for referral ${referral.id}`);
        return rewards;
    }
   ...
   */
    public async voidFutureRewards(referralId: string): Promise<void> {
        ...
        logger.info(`Voided ${snapshot.length} future reward(s) for referral ${referralId}`);
    }
    ...
    private async handlePayout(reward: Reward): Promise<void> {
        if (reward.payoutChannel === PayoutChannel.OFFICERND) {
            ...
            await this.bankPayoutService.issueTransfer(reward.referrerId, reward.amountEur);
        } else if (reward.payoutChannel === PayoutChannel.MANUAL) {
            // TODO: manual payout
        } else {
            throw new AppError('Invalid payout channel', ErrorCode.INVALID_ARGUMENT, 400);
        }
    }
}