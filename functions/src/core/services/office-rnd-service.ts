...
export default class OfficeRndService {
  ...
  private async initializeToken(): Promise<void> {
    try {
      ...
      if (this.token == null) {
        throw new AppError('OfficeRndService.initializeToken()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
      }

      // Check if token is expired
      const now = new Date();
      if (!this.token.updated_at) {
        throw new AppError('Office Rnd token updated_at field is null', ErrorCode.UNKNOWN_ERROR, 500);
      }
      ...
      logger.warn('OfficeRndService.initializeToken()- Error initializing token', { error });
  ...
  public async _getAndSaveToken(
    ...
    if (secretKey == null || secretKey == '') {
      throw new AppError('OfficeRndService.getAndSaveToken() - Secret key is empty', ErrorCode.UNKNOWN_ERROR, 500);
    }
  ...
  public async getMembersByEmail(
    ...
    logger.info('OfficeRndService.getMembersByEmail() - Getting members by email', { email: email });
    // Initialize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.getMembersByEmail()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    const body = await response.json();
    if (response.status !== 200) {
      throw new AppError('OfficeRndService.getMembersByEmail()- Failed to get Office Rnd members by email', ErrorCode.UNKNOWN_ERROR, 500, body);
    }
  ...
  public async updateMember(id: string, properties: OfficeRndMemberProperties) {
    logger.info('OfficeRndService.updateMember() - Updating member', { id: id, properties: properties });
    // initilize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.updateMember()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    // Update the member.
    const url = `${officeRndConfig.apiV2url}/${officeRndConfig.orgSlug}/members/${id}`;
    const options = {
    ...
    const body = await response.json();
    if (response.status !== 200) {
      throw new AppError('OfficeRndService.updateMember()- Failed to update Office Rnd member', ErrorCode.UNKNOWN_ERROR, 500, body);
    }
    return;
  }
  ...
  public async getLocations(): Promise<Array<OfficeRndLocation>> {
    ...
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.getLocations()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    const body = await response.json();
    if (response.status !== 200) {
      throw new AppError('OfficeRndService.getLocations()- Failed to get Office Rnd locations', ErrorCode.UNKNOWN_ERROR, 500, body);
    }
  ...
  public async createMember(
    ...
    logger.info('OfficeRndService.createMember() - Creating member', { member: member });

    // initilize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.createMember()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    if (response.status !== 201) {
      logger.warn('OfficeRndService.createMember()- status is not 201', { status: response.status });
      throw new AppError('OfficeRndService.createMember()- Failed to create Office Rnd member', ErrorCode.UNKNOWN_ERROR, 500, body);
    }
  ...
  public async getOpportunityStatuses(
  ): Promise<Array<OfficeRndOpportunityStatus>> {
    logger.info('OfficeRndService.getOpportunityStatuses() - Getting opportunity statuses');
    // initilize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.getOpportunityStatuses()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    try {
      ...
      const opportunityStatuses
        : Array<OfficeRndOpportunityStatus> = body.results;

      // Save to Firestore
      await this.params.firestoreService
        .setDocuments(opportunityStatuses.map((status) => ({
          collection: this.officeRndCollection,
  ...
  public async getOpportunities(params: {
    ...
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.getOpportunities()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    const body = await response.json();
    if (response.status !== 200) {
      throw new AppError('OfficeRndService.getOpportunities()- Failed to get Office Rnd opportunities', ErrorCode.UNKNOWN_ERROR, 500, body);
    }
  ...
  public async updateOpportunity(
    ...
    logger.info('OfficeRndService.updateOpportunity() - Updating opportunity', { id: id, opportunity: opportunity });
    // initilize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.updateOpportunity()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
    ...
    const body = await response.json();
    if (response.status !== 200) {
      throw new AppError('OfficeRndService.updateOpportunity()- Failed to update Office Rnd opportunity', ErrorCode.UNKNOWN_ERROR, response.status, body);
    }
    return;
  }
  ...
  public async createOpportunity(
    ...
    logger.info('OfficeRndService.createOpportunity() - Creating opportunity', { opportunity: opportunity });
    // initilize token.
    await this.initializeToken();
    if (this.token == null) {
      throw new AppError('OfficeRndService.createOpportunity()- Office Rnd token is null', ErrorCode.UNKNOWN_ERROR, 500);
    }
